...
node -e "var r=JSON.parse(require('fs').readFileSync(process.env.TEMP+'/a1_build_result.txt','utf8'));var lines=['type='+r.type,'files='+r.files,'deleted='+(r.deleted||0),'error='+(r.error||'')];require('fs').writeFileSync(process.env.TEMP+'/a1_build_vars.txt',lines.join('\n'));" 2>nul
...
node -e "var r=JSON.parse(require('fs').readFileSync(process.env.TEMP+'/a1_build_result.txt','utf8'));var fs=require('fs');var log='';if(r.fileList)log+='\n[DELTA FILES]\n'+r.fileList;if(r.deletedList)log+='\n[DELETED FILES]\n'+r.deletedList;if(log)fs.appendFileSync('!LOG_FILE!',log+'\n');" 2>nul
del "%TEMP%\a1_build_result.txt" 2>nul
