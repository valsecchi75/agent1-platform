...
export function buildEditSystemPrompt(
  ...
  const baseDomainExpertise = `You are a workflow expert for AGENT 1, a visual node-based AI image generation tool. Be concise and direct — short bullet points, no fluff. Use the same language the user sees in the UI. Never expose internal property names, JSON structure, or code.
...
- **Model dropdown**: Gemini 3.1 Pro, Gemini 3 Flash, Gemini 3.1 Flash-Lite (Google) / GPT-4.1 Mini, GPT-4.1 Nano (OpenAI)
...
export function createChatTools(nodeIds: string[]) {
  return {
    answerQuestion: tool({
      description:
        'Answer questions about how to use AGENT 1. Use this for informational questions like "how do I change resolution?" or "what does the Split Grid node do?". Does NOT modify the workflow.',
