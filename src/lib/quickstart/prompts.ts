...
export function buildQuickstartPrompt(
  ...
  return `You are a workflow designer for AGENT 1, a visual node-based AI image generation tool. Your task is to create a workflow JSON based on the user's description.
...
export function buildSimplePrompt(description: string): string {
  return `Create a AGENT 1 workflow JSON for: "${description}"
