...
export async function executeNanoBanana(
  ...
  try {
    ...
    if (result.success && result.image) {
      ...
      // Calculate cost for DB record
      let saveCostUsd = 0;
      if (nodeData.selectedModel?.provider === "fal" && nodeData.selectedModel?.pricing) {
        saveCostUsd = nodeData.selectedModel.pricing.amount;
      } else if (!nodeData.selectedModel || nodeData.selectedModel.provider === "gemini") {
        saveCostUsd = calculateGenerationCost(nodeData.model, nodeData.resolution);
      }

      // Auto-save to AGENT 1 output folder (agent1_XXXX naming)
      fetch("/api/agent1-save", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          provider: nodeData.selectedModel?.provider || "gemini",
          aspectRatio: nodeData.aspectRatio || "1:1",
          resolution: nodeData.resolution || "unknown",
          costUsd: saveCostUsd,
          workflowId: workflowId || undefined,
          workflowName: workflowName || undefined,
        }),
      }).catch((err) => console.error("AGENT 1 auto-save failed:", err));

      // Save the current workflow state for the "Last Workflow" button
      try {
        const { persistLastGenerationWorkflow } = await import("@/store/tabStore");
        persistLastGenerationWorkflow();
