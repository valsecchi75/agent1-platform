  ...
  try {
    ...
    if (result.success && (videoData || result.image)) {
      ...
      if (videoData) {
        ...
        let videoCostUsd = 0;
        if (nodeData.selectedModel?.provider === "fal" && nodeData.selectedModel?.pricing) {
          videoCostUsd = nodeData.selectedModel.pricing.amount;
        }
        ...
        fetch("/api/agent1-save", {
          ...
          body: JSON.stringify({
            ...
            data: videoData,
            prompt: text || "",
            model: nodeData.selectedModel?.modelId || "unknown",
            provider: nodeData.selectedModel?.provider || "unknown",
            costUsd: videoCostUsd,
            workflowId: workflowId || undefined,
            workflowName: workflowName || undefined,
          }),
        }).catch((err) => console.error("AGENT 1 video auto-save failed:", err));
