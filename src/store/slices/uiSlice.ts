...
export interface UISlice {
  ...
  updateProgress: {
    ...
    isUpdating: boolean;
    error: string | null;
    progress: number | null; // 0-100 download percentage
  };
...
export const createUISlice: StateCreator<UISlice, [], [], UISlice> = (set, get) => ({
  ...
  updateInfo: null,
  updateProgress: { step: null, status: null, isUpdating: false, error: null, progress: null },
  updateDismissed: false,
  updateSkippedVersion: typeof window !== 'undefined'
    ? localStorage.getItem('agent1-update-skip-version')
    : null,
  ...
  skipUpdateVersion: (version: string) => {
    try {
      if (typeof window !== 'undefined') {
        localStorage.setItem('agent1-update-skip-version', version);
      }
    } catch { /* localStorage may be unavailable — skip persists only in-memory */ }
