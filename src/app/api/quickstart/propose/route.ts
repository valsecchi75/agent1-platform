...
import { buildProposalPrompt, buildRAGProposalPrompt } from "@/lib/quickstart/proposalPrompt";
import { parseJSONFromResponse } from "@/lib/quickstart/validation";
import { getNodeIndex } from "@/lib/nodeIndex";
...
function validateProposalShape(data: unknown): string | null {
  ...
  // All valid node types (full set from NodeType union in types/nodes.ts)
  const validNodeTypes: NodeType[] = [
    "imageInput", "audioInput", "annotation", "prompt", "array",
    "promptConstructor", "nanoBanana", "generateVideo", "generateAudio",
    "generate3d", "llmGenerate", "splitGrid", "output", "outputGallery",
    ...
    "naSketchToPhoto", "naStylingDetail", "naRecolor",
  ];
  ...
  const validConnectionTypes = ["image", "text", "audio", "video", "3d", "reference", "easeCurve"];
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Build the proposal prompt (RAG-aware if node index is available)
    let prompt: string;
    try {
      const nodeIndex = await getNodeIndex();
      if (nodeIndex && nodeIndex.nodeCount > 0) {
        prompt = buildRAGProposalPrompt(description.trim(), nodeIndex);
        console.log(`[Propose:${requestId}] RAG prompt built (${nodeIndex.nodeCount} nodes), length: ${prompt.length}`);
      } else {
        prompt = buildProposalPrompt(description.trim());
        console.log(`[Propose:${requestId}] Legacy prompt built (empty index), length: ${prompt.length}`);
      }
    } catch {
      prompt = buildProposalPrompt(description.trim());
      console.log(`[Propose:${requestId}] Legacy prompt built (index load failed), length: ${prompt.length}`);
    }
