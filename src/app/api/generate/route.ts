...
export async function POST(request: NextRequest) {
  ...
  // ── Budget pre-check (non-blocking if DB unavailable) ──────────────
  try {
    const { getRequestUser } = await import("@/lib/auth/getRequestUser");
    ...
    if (dbUser) {
      const result = checkBudget({
        ...
        role: user.role,
        estimatedCost: getEstimatedCost(null),
      });
      if (!result.allowed) {
        console.log(`[API:${requestId}] Budget check BLOCKED: ${result.budgetWarning}`);
        return NextResponse.json<GenerateResponse>(
          { success: false, error: result.budgetWarning || "Department budget exceeded" },
          { status: 403 }
        );
      }
      if (result.budgetWarning) {
        console.log(`[API:${requestId}] Budget warning: ${result.budgetWarning}`);
      }
    }
  } catch (budgetErr) {
    // Auth or DB unavailable — allow generation to proceed
    console.warn(`[API:${requestId}] Budget pre-check skipped:`, budgetErr);
  }
