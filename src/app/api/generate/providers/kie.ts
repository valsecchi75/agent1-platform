...
import { parseDataUrl } from "@/lib/api";
...
export async function uploadImageToKie(
  ...
  // Extract mime type and data from data URL using shared utility
  const parsed = parseDataUrl(base64Image);
  const declaredMimeType = parsed.mimeType;
  const imageData = parsed.base64;

  // Get binary data from parsed result
  const binaryData = parsed.buffer;
