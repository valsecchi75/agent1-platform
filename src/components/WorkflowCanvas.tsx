...
import { getHandleType, getNodeHandles, validateConnectionWithSpec } from "@/utils/connectionValidation";
...
export function WorkflowCanvas() {
  ...
  const onboardingActive = useOnboardingStore((state) => state.showWizard);
  ...
  // Connection validation — delegates to spec-driven validateConnectionWithSpec()
  // Defined inside component to have access to nodes array via nodeMap
  const isValidConnection = useCallback(
    (connection: Connection | Edge): boolean => {
      const source = (connection as { source: string }).source;
      const target = (connection as { target: string }).target;
      const sourceNode = nodeMap.get(source);
      const targetNode = nodeMap.get(target);
      return validateConnectionWithSpec(
        {
          source,
          target,
        ...
        sourceNode ? { id: sourceNode.id, type: sourceNode.type, data: sourceNode.data as Record<string, unknown> } : undefined,
        targetNode ? { id: targetNode.id, type: targetNode.type, data: targetNode.data as Record<string, unknown> } : undefined
      );
    },
    [nodeMap]
  );
  ...
  return (
    ...
    >
      ...
      {/* Welcome Modal — hidden while onboarding wizard is active */}
      {showQuickstart && !onboardingActive && (
        <WelcomeModal
