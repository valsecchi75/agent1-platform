...
type RecolorNodeType = Node<NARecolorData, "naRecolor">;

export function NARecolorNode({ id, data, selected }: NodeProps<RecolorNodeType>) {
  ...
  return (
    <BaseNode id={id} selected={selected} isExecuting={isLoading} hasError={hasError} fullBleed aspectFitMedia={data.outputImage}>
      ...
      <Handle type="target" position={Position.Left} id="color_reference_image" style={{ top: "65%", zIndex: 10 }} data-handletype="image" isConnectable={true} />
      <div className="handle-label absolute text-[10px] font-medium whitespace-nowrap pointer-events-none text-right"
        style={{ right: "calc(100% + 8px)", top: "calc(65% - 7px)", color: "var(--handle-color-image)", opacity: 0.6, zIndex: 10 }}>Color Ref</div>
      ...
      <div className="relative w-full h-full min-h-0 overflow-hidden rounded-lg">
        {data.outputImage ? (
          <>
            <img src={adaptiveImage ?? undefined} alt="Recolored" className="w-full h-full object-cover" />
