...
// R9.5: Memoize ImageCompareNode to prevent re-renders unless id, data, or selected changes
function ImageCompareNodeComponent({
  id,
  data,
  selected,
  ...
  return (
    ...
    >
      ...
      />
      <div className="handle-label absolute text-[10px] font-medium whitespace-nowrap pointer-events-none text-right"
        style={{ right: "calc(100% + 8px)", top: "calc(65% - 7px)", color: "var(--handle-color-image)", zIndex: 10 }}>
        Image B
      </div>

      {/* Output handle for comparison results */}
      <Handle
        type="source"
      ...
      {imageA && imageB ? (
        <div className="flex-1 relative nodrag nopan nowheel">
          ...
          />
          {/* Corner labels */}
          <div className="absolute top-2 left-2 bg-black/50 text-neutral-50 text-[10px] font-medium px-2 py-1 rounded pointer-events-none">
            A
          </div>
          <div className="absolute top-2 right-2 bg-black/50 text-neutral-50 text-[10px] font-medium px-2 py-1 rounded pointer-events-none">
            B
          </div>
        </div>
      ) : (
