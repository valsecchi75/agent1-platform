      ...
      {nodeData.inputType ? (
        <div>
          <Handle
            ...
            style={{ top: baseOffset, zIndex: 10 }}
          />
          <div className="handle-label absolute text-[10px] font-medium whitespace-nowrap pointer-events-none text-right"
            style={{ right: "calc(100% + 8px)", top: baseOffset - 7, color: HANDLE_COLORS[nodeData.inputType], zIndex: 10 }}>
            {nodeData.inputType}
          </div>
        </div>
      ) : (
        <div>
          ...
          />
          <div className="handle-label absolute text-[10px] font-medium whitespace-nowrap pointer-events-none text-right"
            style={{ right: "calc(100% + 8px)", top: baseOffset - 7, color: "#6b7280", zIndex: 10 }}>
            Input
          </div>
        </div>
      )}
        ...
        nodeData.switches.map((sw, index) => (
          <div key={`output-${sw.id}`}>
            ...
            <div className="handle-label absolute text-[10px] font-medium whitespace-nowrap pointer-events-none"
              style={{ left: "calc(100% + 8px)", top: baseOffset + index * handleSpacing - 7, color: HANDLE_COLORS[nodeData.inputType!], opacity: sw.enabled ? 1 : 0.3, zIndex: 10 }}>
              {sw.name}
            </div>
          </div>
        ))}
      ...
      <div className="px-2 py-1 space-y-1">
        {!showOutputs ? (
          ...
          <>
              ...
              >
                ...
                <label className="relative inline-flex items-center cursor-pointer">
                  ...
                  <div className="w-8 h-4 bg-neutral-600 peer-checked:bg-violet-500 rounded-full transition-colors relative">
                    <div className={`absolute top-0.5 left-0.5 bg-[var(--surface-1)] h-3 w-3 rounded-full transition-transform ${sw.enabled ? "translate-x-4" : ""}`} />
                  </div>
                </label>
            ...
            {/* Add switch button */}
            <button
              className="w-full flex items-center justify-center gap-1 text-neutral-400 hover:text-neutral-50 text-xs py-1 mt-2 rounded hover:bg-violet-900/30 transition-colors"
