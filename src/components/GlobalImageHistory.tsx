...
function FanItem({
  ...
  return (
    <button
      ...
      className="absolute w-14 h-14 rounded-lg overflow-hidden border-2 border-neutral-600 hover:border-[var(--accent)] shadow-lg cursor-grab active:cursor-grabbing transition-colors duration-150 animate-fan-enter group"
      style={
        {
          "--fan-x": `${x}px`,
          "--fan-y": `${y}px`,
    ...
    >
      ...
      <div className="px-4 py-3 border-b border-neutral-700 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={onClose}
            className="w-5 h-5 rounded hover:bg-neutral-700 flex items-center justify-center text-neutral-400 hover:text-neutral-50 transition-colors"
            title="Close"
          >
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1.5">
        {history.map((item, index) => (
          ...
          >
            {/* Thumbnail */}
            <div className="w-14 h-14 rounded overflow-hidden shrink-0 border border-neutral-600 group-hover:border-[var(--accent)] transition-colors">
              <img
                src={item.image}
                alt={`History ${index + 1}`}
                className="w-full h-full object-cover pointer-events-none"
  ...
  return (
    <div ref={drawerRef} className="absolute bottom-4 right-64 z-10">
      ...
      >
        ...
        {/* Badge showing count */}
        <span className="absolute -top-1.5 -right-1.5 min-w-[18px] h-[18px] px-1 bg-[var(--accent)] rounded-full text-[10px] text-[var(--btn-primary-text)] flex items-center justify-center font-bold">
          ...
          {hasOverflow && (() => {
            ...
            return (
              <button
                ...
                className="absolute animate-fan-enter bg-neutral-800 hover:bg-neutral-700 border border-neutral-600 rounded-lg px-2 py-1 text-[10px] text-neutral-300 hover:text-neutral-50 shadow-lg transition-colors whitespace-nowrap"
                style={
                  {
